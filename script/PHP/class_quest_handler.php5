...
class classQuestHandler
{
    ...
    function addStationIncludes($xPath, $xhtml)
    {
        ...
        for ($i = 0; $i < $countResult; $i++)
        {
            ...
            {
                $resultIn = $xPath->match("in", $result[$i]);
                if ( $this->getNodeState($xPath, $resultIn[0]) )
                {
                    ...
                    {
                        $thisResult = $xPath->match('./../../.', $resultIn[0]);
    ...
    {
        $newContext = null;
        $result = $xPath->match('if', $context);
        $countResult = count($result);
        if ($countResult > 0)
        {
            for ($i = 0; $i < $countResult && $newContext == null; $i++)
            {
                if ( $this->getNodeState($xPath, $result[$i]) )
                {
                    $this->debugInfo .= "ifResult=" . $xPath->getData($result[$i]) . "<br />";
                    $newContext = $result[$i];
                }
            }
            if ($newContext == null)
                $newContext = $xPath->match('else', $context);
        }
        else
            $newContext = $context;
    ...
    function getNodeState($xPath, $result)
    {
        ...
        $retval = $this->getNodeStateByValue($checkValue);

        if ($retval == '') { $retval = 0; }

        return intval($retval) == 1;
    }
